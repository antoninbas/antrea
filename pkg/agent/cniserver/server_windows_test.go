...
func TestCmdDel(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.ifaceExists {
				containerIface := interfacestore.NewContainerInterface(ovsPortName, tc.containerID, tc.podName, testPodNamespace, ipamType, containerMAC, []net.IP{net.ParseIP("10.1.2.100")}, 0)
...
func TestCmdCheck(t *testing.T) {
	...
	wrapperContainerInterface := func(ifaceName, containerID, podName, ovsPortID string, mac net.HardwareAddr, containerIP net.IP) *interfacestore.InterfaceConfig {
		containerIface := interfacestore.NewContainerInterface(ifaceName, containerID, podName, testPodNamespace, ipamType, mac, []net.IP{containerIP}, 0)
		containerIface.OVSPortConfig = &interfacestore.OVSPortConfig{
			PortUUID: ovsPortID,
			OFPort:   10,
		}
